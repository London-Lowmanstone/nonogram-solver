...
-- This is meant to be used on an iPad with the app Codea.
-- Here "nonograms" are called Nonologix because I built this app to solve the puzzles given by the app "Nono Logix"
...
function setup()
    ...
    --[[
    ans = solver:getUpdatedBand({0,2,2,2,2,2,2,2,2,2}, solver:generatePosses({1,3, 2}, 10))
    for i=10,10 do
        print(i)
    end
    printStuff(ans)
    ...
    --[[
    solver:generatePosses({1,3, 2}, 10)
    for i, val in pairs(ans) do
        printStuff(i, val)
    end
    ]]
    --[[
    solver.toCheck = {{9},{5}}
    solver:updateChecks({2,2,2,2,2}, {2,0,1,1,2}, 2)
    printStuff(solver.toCheck)
      ]]
    --[[
    print(loadstring("return {1,2,3"))
    printStuff(solver:solve({{1,5},{4},{1,2,2},{4,1},{4},{1,1,3},{3,1},{3},{2,3,2},{6,2}},
    {{1,1,1,2},{2},{1,1},{4,5},{5,4},{2,3,3},{2,4},{1,2},{2,2},{1,2}}))
    print("done")
    ans = solver:solve({{1,5},{4},{1,2,2},{4,1},{4},{1,1,3},{3,1},{3},{2,3,2},{6,2}},
    {{1,1,1,2},{2},{1,1},{4,5},{5,4},{2,3,3},{2,4},{1,2},{2,2},{1,2}})
      ]]
    input = {{},{}}
    inputMode = 0
...
Memory for the actual update, not just the generation of all possibilities

Memory for each bandpuzzle (since its only going to get smaller). I think I'll do this.

if ipairs is actually faster for lists, then use ipairs more often

make the possibility generation/init also include a first primer for the board.
  ]]
